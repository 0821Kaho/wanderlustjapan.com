...
export default function SearchButton() {
  ...
  const getSearchResults = (): SearchResult[] => {
    ...
    return results.slice(0, 5); // Max 5件
  };
  ...
  return (
    <div className="relative">
      ...
      {/*
        ここでは「DOMを常に置いておく」→ isOpenで見た目だけ切り替え
        motion.div(Framer Motion)を使い、AnimatePresenceは使わない。
        “initial={false}”で SSR時とClient初回時の状態を一致させる。
      */}
      <motion.div
        className="absolute left-0 top-full mt-2 w-screen max-w-md 
                   rounded-lg bg-white/10 backdrop-blur-md
                   border border-white/20 p-4 shadow-xl"
        initial={false}
        animate={
          isOpen
      ...
      >
        ...
        <div className="relative mb-4">
          <input
            ...
            className="w-full rounded-lg bg-white/20 border border-white/30
                       px-4 py-2 text-white placeholder-white/60
        ...
        {searchResults.length > 0 ? (
          ...
          <p className="text-center text-sm text-white/60">No results found</p>
        ) : null}
      </motion.div>
    </div>
  );
